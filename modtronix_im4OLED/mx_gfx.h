 ...
 *
 */
class MxGfx : public Stream
{
 public:
    MxGfx(int16_t w, int16_t h)
        : _rawWidth(w)
        , _rawHeight(h)
        , _width(w)
        , _height(h)
    ...
    // this is optional
    virtual uint8_t invertDisplay(bool i) {return 0; };
    ...
    /** Set the text cursor location, based on the size of the text. Following commands will write text
     * to the RIGHT and BELOW this given x,y Coordinate. For example, use 0,0 to write text to top,left
     * of display.
     * @param x X coordinate, a value from 0 - (width-1)
     * @param y Y coordinate, a value from 0 - (height-1)
     */
...
protected:
    ...
    bool     dirty; // Set to true if display changes, and needs to be updated. display() function will check and reset this flag
};
